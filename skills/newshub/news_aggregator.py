#!/usr/bin/env python3
"""
Global News Aggregator Script
Fetches news from multiple APIs and generates HTML report
"""
    ...
    <div class="container">
        <div class="header">
            ...
            <div class="stats">
                ...
                <div class="stat">
                    <div class="stat-number">{len([n for n in self.all_news if n['source_type'] == 'International'])}</div>
                    <div class="stat-label">国际新闻</div>
                </div>
                <div class="stat">
                    <div class="stat-number">{len([n for n in self.all_news if n['source_type'] == 'Domestic'])}</div>
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python news_aggregator.py <config_path> [output_path]")
        sys.exit(1)
    ...
    try:
        aggregator = NewsAggregator(config_path)
