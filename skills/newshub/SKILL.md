...
3. **缺口自愈** — 被剔除的空位按分区向模型追加补稿（最多 3 轮），补稿时附上
   ...
   瞬时限流会先退避重试，仍失败则跳过补稿，以现有条数继续）；只有首轮生成失败才
   立即终止（此时没有可发布内容）。

4. **发布门槛：目标 20 条，不足也照发** — 终稿只要 ≥ `MIN_PUBLISH_TOTAL`（默认 8）
   条且无硬性错误，就照常出稿推送，缺口仅打印告警（E1/E6 降级为 WARN）。
   超量（>20）先按分区上限 7/7/6 截尾，不因模型多给两条而空窗。
   只有低于下限（说明检索或生成链路异常）或仍存在硬错误时才退出码 1、当天不产出。
   `MIN_PUBLISH_TOTAL=1` 表示"只要能出稿就发"，彻底不允许空窗。
...
| `MIN_PUBLISH_TOTAL` | 否 | 发布下限（默认 8）；不足 20 但 ≥此值即照发，1 = 只要能出稿就发 |
| `MIN_TOTAL` | 否 | `check_report.py` 的发布下限（默认 8），语义同上 |
